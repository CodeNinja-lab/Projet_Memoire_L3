...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-fluid page-header py-5">
            <h1 class="text-center text-white display-6">Faire Votre Commande</h1>
            <ol class="breadcrumb justify-content-center mb-0">
                <li class="breadcrumb-item"><a href="Accueil.html">Accueil</a></li>
                <li class="breadcrumb-item"><a href="page_offres.php">Retour</a></li>
...
// Vérifiez si l'utilisateur est connecté en tant qu'acheteur
if (isset($_SESSION['login'])) {
    ...
    try {
        ...
        // Vérifiez si l'acheteur existe dans la table des acheteurs
        $checkAcheteurQuery = "SELECT COUNT(*) AS acheteurCount FROM acheteur WHERE login = :login_acheteur";
        $checkAcheteurStmt = $conn->prepare($checkAcheteurQuery);
        $checkAcheteurStmt->bindParam(':login_acheteur', $loginAcheteur, PDO::PARAM_STR);
        $checkAcheteurStmt->execute();
        $acheteurCount = $checkAcheteurStmt->fetch(PDO::FETCH_ASSOC)['acheteurCount'];

        if ($acheteurCount > 0) {
            // L'acheteur existe, vous pouvez continuer avec la commande
            if (isset($_GET['id']) && !empty($_GET['id'])) {
                ...
                if ($produit) {
                    ...
                    echo '<h2 class="text-primary mt-5">Commande du produit : ' . $produit['nom'] . '</h2>';
                    echo '<p class="mb-4"><strong>Description : </strong>' . $produit['description'] . '</p>';
                    echo '<p class="mb-4"><strong>Prix :</strong> ' . $produit['prix_par_kg'] . '</p>';
        
                    // Ajoutez un formulaire pour que l'acheteur spécifie la quantité, l'adresse, etc.
                    echo '<form action="traitement_commande.php" method="post" class="mt-4">';
                    echo '   <div class="mb-3">';
                    echo '     <strong>  <label for="quantite" class="form-label">Quantité :</label></strong>';
                    echo '       <input type="number" name="quantite" id="quantite" class="form-control" required>';
                    echo '   </div>';
                    echo '   <div class="mb-3">';
                    echo '   <strong>    <label for="adresse" class="form-label">Adresse de livraison :</label></strong>';
                    echo '       <textarea name="adresse" id="adresse" class="form-control" rows="3" required></textarea>';
                    echo '   </div>';
                    echo '   <input type="hidden" name="produit_id" value="' . $produitId . '">';
            ...
            // L'acheteur n'existe pas dans la table des acheteurs, affichez un message d'erreur
            echo "<script>
            alert( 'Vous devez être connecté ou vous inscrire en tant qu\'acheteur pour commander.');
    ...
    // Affichez un message d'erreur si l'acheteur n'est pas connecté
    echo 'Vous devez être connecté en tant qu\'acheteur pour commander.';
}
?>
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-3 col-md-6">
                        <div class="d-flex flex-column text-start footer-item">
                            ...
                            <a class="btn-link" href="ProfilAcheteur.php">Mon compte</a>
