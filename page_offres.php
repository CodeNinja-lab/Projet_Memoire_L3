...
<html lang="en">
    ...
    <body>
        ...
        <div class="container-fluid page-header py-5">
            <h1 class="text-center text-white display-6">Les Offres</h1>
...
// Vérifiez si le nom du produit est présent dans l'URL
if (isset($_GET['nom_produit']) && !empty($_GET['nom_produit'])) {
    $nomProduit = urldecode($_GET['nom_produit']);
    ...
    try {
        ...
        $checkProductQuery = "SELECT COUNT(*) AS productCount FROM offres WHERE nom = :nom_produit";
        $checkProductStmt = $conn->prepare($checkProductQuery);
        $checkProductStmt->bindParam(':nom_produit', $nomProduit, PDO::PARAM_STR);
        $checkProductStmt->execute();
        $productCount = $checkProductStmt->fetch(PDO::FETCH_ASSOC)['productCount'];

        if ($productCount > 0) {
            // Récupérez les offres associées au nom du produit
            $sql = "SELECT offres.*, agriculteur.login AS agriculteur_login, agriculteur.nom AS agriculteur_nom, agriculteur.prenom AS agriculteur_prenom, agriculteur.address AS agriculteur_adresse
            FROM offres
            JOIN agriculteur ON offres.login = agriculteur.login
            WHERE offres.nom = :nom_produit";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':nom_produit', $nomProduit, PDO::PARAM_STR);
            $stmt->execute();
            ...
            echo '<div class="row g-4">';
            echo '<h2>Offres disponibles pour le produit ' . $nomProduit . '</h2>';
            echo '<div class="row">';
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo '<img src="' . $row['images'] . '" class="img-fluid w-100 rounded-top" style="max-width: 100%; max-height: 300px;" alt="">';


                echo '</div>';
                echo '<div class="text-white bg-secondary px-3 py-1 rounded position-absolute" style="top: 10px; left: 10px;">' . $row['categorie'] . '</div>';
                echo '<div class="p-4 border border-secondary border-top-0 rounded-bottom">';
                echo '<h4>Nom du Produit : '.'<em>'.$row['nom'].'</em>'.'</h4>' ;
                echo '<h4>Prix par KG : '.'<em>'.$row['prix_par_kg'].' Francs</em>'.'</h4>' ;
                echo '<h4>Agriculteur : '.'<em>' . $row['agriculteur_prenom'] . ' ' . $row['agriculteur_nom'] .'</em>'. '</h4>';
                echo '<h4>Adresse :' . $row['agriculteur_adresse'] . '</h4> ';
                echo '<div class="d-flex flex-column">';
                echo '<button class="btn btn-primary rounded-pill mb-2" onclick="commanderProduit(' . $row['id'] . ')">Commander</button>';
                echo '<a href="details_produit.php?id=' . $row['id'] . '" class="btn btn-secondary rounded-pill">Voir les détails du produit</a>';          
                echo '</div>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            ...
            echo 'Il n\'existe pas encore d\'offre pour le produit avec le nom ' . $nomProduit . '.';

        }
    } catch (PDOException $e) {
        echo 'Erreur de récupération des offres: ' . $e->getMessage();
    }

    // Fermez la connexion à la base de données
    $conn = null;
} else {
    echo 'Nom du produit non valide.';
}
...
<!-- Profil de l'agriculteur End -->
    ...
    // Vous pouvez rediriger l'utilisateur vers une page de confirmation ou un formulaire de commande
    window.location.href = 'commande.php?id=' + produitId;
}</script>
    </body>
